...
2. **The dashboard content doesn't match.** Figma's home screen has: a slim red header with a greeting + user avatar + notification bell; a dark "Asistente de Préstamos AI" banner card; a white account balance card (bank name, masked account number, available balance, CLABE with copy affordance); and a 3-icon "Operaciones rápidas" row (Ahorros, Préstamos, Transferir). The current dashboard has a plain greeting and two generic list-style action cards — none of the above.
