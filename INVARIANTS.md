...
- **INV-019 — Database backend abstraction.** Persistence is reached only through a repository port with interchangeable adapters: `local_sqlite` (local SQLite file, development) and `d1_http` (Cloudflare D1 via the Cloudflare REST API, showcase). Selection is environment-only (`DB_BACKEND`); no source change may be required to switch. The backend never assumes a Cloudflare Workers binding. If D1 is unreachable or over quota, the system falls back to the local seeded SQLite backend so the demo survives; the fallback is always traced and never silent. Schema, migrations, and seed must stay within the D1-compatible SQLite subset.

## C. Data & interface invariants

- **INV-020 — Storage.** All data lives in **SQLite** through the repository abstraction (INV-019): a local SQLite file for development and **Cloudflare D1** for the showcase. All data is mocked/seeded. No real banking integrations, no general-purpose production database, no auth system.
...
- **INV-022 — Freshness + revalidation.** Placeholders move *data* only. Before every delivery the backend hydrates placeholders from the configured database backend (INV-019) **and** runs an agent revalidation pass that may mutate *structure*. **Stale or leaked data must never be delivered.**
